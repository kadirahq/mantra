...
However, most of the apps will have server side components as well. So, we've a suggested directory layout for server side as well.
...
When creating UI components you should pass all the data, event handlers via props.
...
You can also import any library function and use them in the UI components. You can import directly them from NPM modules, but not from Meteor packages. These functions should be [pure](https://en.wikipedia.org/wiki/Pure_function).
...
Dependencies will be injected to the top-level components in your app. This could be a Layout component. You can do this inside your routes. See:

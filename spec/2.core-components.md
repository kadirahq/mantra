...
In Mantra, the separation between client and server should be explicit and implicit code sharing is discouraged. If you need to implement latency compensation on the client side, you should write corresponding method stubs for server side methods. See [client/configs/method_stubs/posts.js](https://github.com/mantrajs/mantra-sample-blog-app/blob/master/client/configs/method_stubs/posts.js) and [server/methods/posts.js](https://github.com/mantrajs/mantra-sample-blog-app/blob/master/server/methods/posts.js), note how the client side stub injects a client specific field `saving` which is reflected in the user interface but not persisted to server.

See more about the rationale here: [Say no to universal apps](https://voice.kadira.io/say-no-to-isomorphic-apps-b7b7c419c634)

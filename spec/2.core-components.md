...
Mantra gives special attention to the **client side** of your app. Mantra do not mix client and server code together, instead it recommends code sharing. These are the reasons for it:

* Client side is the place where you put a lot of effort. It's the biggest part of your codebase. Server side codebase is relatively simpler to manage and organize.
* In the future, client side app interact with the server using a schema. Client side app has no idea of how the server has implemented.
* Mantra [does not believe in Universal Apps](https://voice.kadira.io/say-no-to-isomorphic-apps-b7b7c419c634#.hogcs5r24). It encourage multiple apps for different platforms with code sharing. Then it's common to have a single server to interact with a few client apps.

Based on above factors, it's not a good idea to mix client and server code together.

**When we discuss about Mantra, it's about the client side of your app.**

However, most of the apps will have server side components. So, we've a suggested directory layout for server side as well.

For that, refer [Appendix B](#sec-Appendix-Server-Side-Directory-Layout)
...
When creating UI components you should pass all data, event handlers via props.
...
You can also import any library function and use them in the UI components. You can import directly them from NPM modules, but not from any Meteor packages. These functions should be [pure](https://en.wikipedia.org/wiki/Pure_function).
...
Note: Inside an action, everything you do should be based on the Application Context and other arguments passed to the action. You should not import any ES2015 module expect [libraries](#sec-Libraries). You should also avoid using Global variables inside actions.
...
Containers are the integration layer in Mantra. They do these things:
...
We have some rules when creating container:
...
import {useDeps, composeWithTracker, composeAll} from 'react-komposer';
...
We use a project called [`react-simple-di`](https://github.com/kadirahq/react-simple-di) that uses React Context behind the scene. It accepts both [Application Context](#sec-Application-Context) and [Actions](#sec-Actions) as dependencies.
...
Application Context can be access within [Containers](#sec-Containers) as well.

### Configuring Dependency Injections

Dependencies will be injected to the top-level components in your app. Usually, it'll be a Layout Component. You can do the injection inside your routes. See:
...
We normally use a Router to mount components into the UI. There could be multiple solutions (for example, [Flow Router](https://github.com/kadirahq/flow-router/) and [React Router](https://github.com/rackt/react-router)).
...
Testing is a core part of Mantra. Mantra helps you to test every part of your application. Rules we've enforced help you to write those tests. You can use familiar tools like [Mocha](https://mochajs.org/), [Chai](http://chaijs.com/) and [Sinon](http://sinonjs.org/) to do testing.
...
Mantra follows a modular architecture. All the components of Mantra expect ["Application Context"](#sec-Application-Context) should reside inside a module.

You can create as many as modules you like and communicate between them via imports.

### Application Context & Modules

[Application Context](#sec-Application-Context) is the core of the application. It needs to be defined in a place which is not belong to any module. All the modules could access Application Context as a dependency and modules should not update the Application Context.

### Module Definition

A Mantra module may contains a definition file. It exposes actions, routes and and a function accepting the context. It is the `index.js` file of a module.
...
If the module have no actions, routes or no need to do any initialization then it's okay to avoid using a definition file. These implicit modules may contains following things:
...
Module containers and UI components should be able to import via ES2015 modules.

### Module Actions

A module can expose actions via namespaces. These namespaces are global to the app and module should take responsibility to make them unique. A module can expose multiple namespaces.

Finally, all these namespaces from each module are merged and can be accessible inside actions and containers.

### Routes

For routing, you could use any routing library. It's okay to have route definitions inside multiple modules if needed.

### Core Module

Mantra is 100% modular and there should be at least a single module in an app. We call this the core module. It's just a simple module, but you load it before any other module. This module is the best place to put
...
There could be multiple ways to organize modules depending on the app. Refer [Appendix C](#sec-Appendix-Organizing-Modules) for few of those methods.

### Sub Modules

Inside a module, you **cannot** have sub modules. This is a decision taken to prevent unnecessary complexity. Otherwise, it's possible to write multiple layers of nested modules and that's very hard to manage.

## Single Entry Point

With Mantra, we want our app to be predictable. Therefore, there is only a single entry point in your app. That's the `client/main.js`.

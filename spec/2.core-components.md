...
Mantra is an application architecture for the **client side** of your app. We do not focus too much on the server side of your app. These are the reasons for it:

* Client side is the place where you develop your app most of the time. Server side codebase is simpler to manage and organize.
* In the future, when interacting with the server, you do it with a schema and client have no idea of what's inside the server.
...
So based on above factors, it's not a good idea to mix client and server code together. That's why Mantra is only for client side apps.

However, most of the apps will have server side components. Even though Mantra is for the client side, we can follow similar concepts to organize server side components.
...
We use React as the UI (presentation) layer in Mantra. You should pass all the data, event handlers via props.
...
You can also import any library function and use them in the UI components. You can import directly them from NPM modules, but not from Meteor packages. These functions should be pure.
...
Note: Inside an action, everything you do should be based on the application context and other arguments passed to the action. You should not import any ES2015 module expect libraries. You should also avoid using Global variables inside actions.
...
* The mapper function should be pure.
...
Dependencies will be injected to the top-level components in your app. This could be a Layout component. You can do this inside yours. See:
...
Every app has some utility functions to do different tasks. You can also get them via NPM. These libraries will export functions which are pure. So, you can import them anywhere in your app including inside actions, components and containers. But they must be pure.
...
Mantra follows a modular architecture. All the components of Mantra expect "Application Context" should reside inside a module.
...
If the module have no actions, routes or no need to do any initialization then it's okay to avoid using a definition file. These modules may contains following things:
...
A module can expose actions via a namespaces. These namespaces are global to the app and module should take responsibility to make it unique. A module can expose multiple namespaces.
...
Inside a module, you can't have sub modules. This is a decision taken to prevent unnecessary complexity. Otherwise, it's possible to write multiple layers of nested modules and that's very hard to manage.

## Single Entry Point

In Mantra, we want our the app to be predictable. Therefore, there is only a single entry point in your app. That's the `client/main.js`.

It'll initialize the context and load all the modules in your app. Here's an example `client/main.js` file.
